...
- **The donut and the tile next to it count different things, on purpose.** The donut is the current library — books with a `files` row, and how many of *those* are `completed` — so it answers "how much of what I have have I read" and drops when new books are copied on. The tile is the all-time finished count from `completed_ts`. They disagree because finished books get deleted; never "fix" one to match the other, and never mix the two halves (26 finished ever ÷ 5 books present is how it once reported 100 %).
