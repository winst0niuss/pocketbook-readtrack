...
`make qt` needs Docker, which this machine does not have: the device binary is built in CI, not locally. So a change that has to reach the reader ships as one commit that bumps `VERSION`, then a `vX.Y.Z` tag pushed after it — CI cross-compiles, checks the tag against `VERSION` and publishes `ReadTrack.zip`, and the reader picks it up from the About screen. `gh run watch <id> --exit-status` is the wait; nothing else here proves the C++ still compiles for ARM.
