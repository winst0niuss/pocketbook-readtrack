...
int quickDownload(const QByteArray &url, const QByteArray &dest)
{
    int size = 0;
    ...
    if (const auto ext3 = resolve<void *(*)(const char *, int *, int, char *,
        ...
        int error = 0;
        void *data = ext3(url.constData(), &size, kTimeoutSeconds, nullptr,
                          nullptr, &error);
    ...
    if (const auto quick =
        ...
        void *data = quick(url.constData(), &size, kTimeoutSeconds);
        const int rc = writeResult(data, size, dest);
        if (data != nullptr)
            free(data);
        return rc;
...
int inkViewDownload(const QString &url, const QString &dest)
{
    ...
    int rc = quickDownload(target, destPath);
    if (rc != NET_OK) {
        /* An empty body is what a dropped association looks like from here, so
         * reconnect and give it one more go before calling it a failure. */
        updateLog(QStringLiteral("net: retrying after %1").arg(rc));
        if (inkViewNetworkUp())
            rc = quickDownload(target, destPath);
    }
